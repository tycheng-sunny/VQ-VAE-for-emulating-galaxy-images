...
class PixelCNN(object):
    ...
    def save(self, sess, savepath, step=None):
        if(step is not None):
            ...
            self.saver.save(sess, savepath+'/last-pixelcnn_noise+psf.ckpt')
...
for i in range(runN):
    if i == 0:
        embeds_indice_4_imgs = sess.run(vq_output_train["encoding_indices"], feed_dict={x: data_dict['images'][i*N: (i+1)*N], x_sigma: data_dict['noise'][i*N: (i+1)*N], x_psf: data_dict['psf'][i*N: (i+1)*N]})
    else:
        tmp = sess.run(vq_output_train["encoding_indices"], feed_dict={x: data_dict['images'][i*N: (i+1)*N], x_sigma: data_dict['noise'][i*N: (i+1)*N], x_psf: data_dict['psf'][i*N: (i+1)*N]})
        embeds_indice_4_imgs = np.concatenate((embeds_indice_4_imgs, tmp), axis=0)

tmp = sess.run(vq_output_train["encoding_indices"], feed_dict={x: data_dict['images'][runN* N:], x_sigma: data_dict['noise'][runN* N:], x_psf: data_dict['psf'][runN* N:]})
embeds_indice_4_imgs = np.concatenate((embeds_indice_4_imgs, tmp), axis=0)
...
try:
    ...
    plt.savefig(output_PATH + 'loss_pixelcnn_noise+psf.eps')
#timer
